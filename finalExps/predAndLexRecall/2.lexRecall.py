...
class ExpPresentation(trial):
	...
	def showTestTrial(self,curTrial, trialIndex,whichPart):
		...
		if self.experiment.validResponses[response]==curTrial['isOld']:
			isRight=1
			print str(self.experiment.validResponses[response])+'_isCorrect'
		else:
			isRight=0
			print str(self.experiment.validResponses[response])+'_isIncorrect'

		fieldVars=[]
		for curField in self.fieldNames:
			#if curField=='Question' and not 'None':
			# 	curTrial['Question']=curTrial['Question'].decode('ascii','ignore').encode('ascii','ignore')
			 #if curField=='compLower' and not 'None':
			# 	curTrial['compLower']=curTrial['compLower'].decode('ascii','ignore').encode('ascii','ignore')
			# if curField=='unrelOppositeGender' and not 'None':
			#  	curTrial['unrelOppositeGender']=curTrial['unrelOppositeGender'].decode('ascii','ignore').encode('ascii','ignore')
			# if curField=='newWord1' and not 'None':
			#  	curTrial['newWord1']=curTrial['newWord1'].decode('ascii','ignore').encode('ascii','ignore')
			# if curField=='newWord2' and not 'None':
			#  	curTrial['newWord2']=curTrial['newWord2'].decode('ascii','ignore').encode('ascii','ignore')
			# if curField=='wrd1' and not 'None':
			# 	curTrial['wrd1']=curTrial['wrd1'].decode('ascii','ignore').encode('ascii','ignore')
			# if curField=='wrd2' and not 'None':
			# 	curTrial['wrd2']=curTrial['wrd2'].decode('ascii','ignore').encode('ascii','ignore')
